...
const ExplosionMechanics = ({done, changeActiveModal}) => {
    const [context, setContext] = useState(<Title style={{marginTop: 12}} level="3" weight="regular">Нажми на фишку в центре, чтобы устроить мега взрыв</Title>)
    const [changeContextCall, setChangeContextCall] = useState(0)

    const changeContext = () => {
        if (changeContextCall === 0) {
            setContext(<Button style={{marginTop: 12}} onClick={() => changeActiveModal("explosionMechanicsModal")} mode="secondary" >Показать разъяснение</Button>)
    ...
    const IntroGameMap = () => {
        ...
        function onCellClick(row, column) {
            if (0 < row < 9 && 0 < column < 9 && (explosions > 0 || row === 2 && column === 2)) {
                ...
                if (map[row - 1][column - 1]['state'] === 1) {
                    ...
                    setTimeout(() => {
                        ...
                        findAnimateIcons()
                        setMap(newMap);
                        if (map[1][1]['state'] === null && !doneCalled && explosions === 6) {
                            doneCalled = true
                            changeContext()
                            done()
                        }
...
export default ExplosionMechanics